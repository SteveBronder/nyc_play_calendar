...
"""Scraper for the Frigid New York ticket listings."""
...
def parse_html(
    ...
    """Parse event listings from FrigidNYC HTML.

    ``fetch`` is an optional callable used to retrieve individual event pages
    when additional details (description/price) are missing from the listing.
    ``rate_limit`` specifies the minimum seconds between fetches.
    """
