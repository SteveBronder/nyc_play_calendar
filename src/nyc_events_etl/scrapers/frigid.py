...
def fetch_events(session: Any | None = None, sleep_secs: float = 1.0) -> List[EventSeries]:
    """Fetch and parse FrigidNYC events with polite pauses."""

    if session is None:
        with urllib.request.urlopen("https://example.com/frigid") as resp:  # pragma: no cover - network
            html = resp.read().decode("utf-8")
    else:
        resp = session.get("https://example.com/frigid")
